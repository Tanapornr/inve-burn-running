...
เมื่อพนักงานกรอกรหัสในหน้าลงทะเบียน ให้กดปุ่ม `ตกลง` ก่อน ระบบจะตรวจรหัสกับ Google Sheet จริง แล้วดึง `คำนำหน้า`, `ชื่อ`, `นามสกุล`, และ `แผนก` ให้อัตโนมัติ

สคริปต์รองรับทั้งกรณีพิมพ์ `018` และ `18` โดยจะเทียบเป็นรหัสเดียวกัน ถ้ารหัสไม่ถูกต้อง ระบบจะแจ้งให้กรอกรหัสพนักงานใหม่
...
ตอนนี้หน้าเว็บตั้งค่า `demoMode: false` แล้ว และโหลดรายชื่อพนักงานจริงจาก Google Sheet โดยตรงสำหรับขั้นตอนค้นหารหัสพนักงาน

สำหรับการบันทึกข้อมูลสมัคร, เข้าสู่ระบบ, ลืมรหัสผ่าน, และบันทึกผลวิ่งลงชีตจริง ต้องใส่ Web App URL ใน `apiUrl`

ถ้ายังไม่ใส่ `apiUrl` จะสามารถค้นหาพนักงานจากชีต `Employees` ได้ แต่จะยังสมัคร/บันทึกผลวิ่งลงชีตจริงไม่ได้

หน้าแรกจะแสดงอันดับจากผู้ที่อยู่ในชีต `Users` เท่านั้น และคำนวณระยะสะสมจากชีต `Runs` โดยแสดง 10 อันดับแรก
